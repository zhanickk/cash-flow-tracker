...
export function useNewDayCashRegister() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Запрет закрывать одну и ту же дату дважды. Именно это дало самую
      ...
      if (closingBusinessDate) {
        const { data: already, error: checkErr } = await supabase
          ...
          .select("business_date")
          .eq("business_date", closingBusinessDate)
          .limit(1);
        if (checkErr) throw checkErr;
        if (already && already.length > 0) {
          throw new Error(
            `Смена за ${closingBusinessDate} уже закрыта — её итог записан в журнал. ` +
              `Повторное закрытие сломает перенос остатка. Если день нужно исправить, откройте его в архиве смен.`,
          );
        }
      }

      // Вторая защита — по остаткам следующего дня. Проверки выше мало: первое
      // нажатие «Нового дня» переводит смену на следующую дату, поэтому второе
      // нажатие «закрывает» уже её, журнала по ней ещё нет, и проверка выше
      // пропускает. Именно так 16 сентября пять нажатий подряд записали пять
      // комплектов остатков. Остатки открываемого дня уникальны — на них и
      // опираемся.
      if (openingBusinessDate) {
        const { data: dup, error: dupErr } = await supabase
          .from("cash_transactions")
          .select("id")
          .eq("kind", "opening")
          .eq("business_date", openingBusinessDate)
          .limit(1);
        if (dupErr) throw dupErr;
        if (dup && dup.length > 0) {
          throw new Error(
            `Остатки на ${openingBusinessDate} уже записаны — смена закрыта. ` +
              `Повторное закрытие продублирует их. Если день нужно исправить, откройте его в архиве смен.`,
          );
        }
      }
