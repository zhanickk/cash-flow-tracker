 ...
 * «Запчасть» сюда намеренно не входит — это реальное имя/кличка контакта в
 * таблице (подтверждено пользователем), а не бухгалтерская категория. */
...
function collectColumn(
  ...
  currency: "KZT" | "USD",
  maxRow: number,
  // Раньше было 80 — на листах с длинным списком контактов это молча
  // обрезало часть строк ниже лимита без единой ошибки. Подняли с запасом:
  // реальный лист никогда не превышает пары тысяч строк.
  maxScan = 2000,
): ParsedBalanceRow[] {
